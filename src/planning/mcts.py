"""Monte Carlo Tree Search for fully observable simulators."""
...
class MCTSPlanner:
    """Simple MCTS planner for gym-style simulators."""
    ...
    def _simulate(self, env, node: _Node, depth: int) -> float:
        ...
        if len(node.children) < len(self.actions):
            ...
            if not done and depth + 1 < self.max_depth:
                rollout_return += self.gamma * self._rollout(env, depth + 1)
            self._backup(node.children[action], rollout_return)
